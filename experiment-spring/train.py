...
from hnn import HNN
from data import get_dataset, kofmotion, hamiltonian_fn
...
def get_args():
    ...
    parser.add_argument('--field_type', default='contact_field', type=str, help='type of vector field to learn')
    # here we have a trouble to solve
...
def train(args):
  ...
  if args.verbose:
    ...
    if args.hamint:
      print("Hamiltonian Integrator Training")
    
  output_dim = args.input_dim if args.baseline else 2
  nn_model = MLP(args.input_dim, args.hidden_dim, output_dim, args.nonlinearity)
  model = HNN(args.input_dim, differentiable_model=nn_model,
              field_type=args.field_type, baseline=args.baseline)
  optim = torch.optim.Adam(model.parameters(), args.learn_rate, weight_decay=1e-3)

  # arrange data
  data = get_dataset(hamint=args.hamint, seed=args.seed)
  x = torch.tensor( data['x'], requires_grad=True, dtype=torch.float32)
  test_x = torch.tensor( data['test_x'], requires_grad=True, dtype=torch.float32)
  dxdt = torch.Tensor(data['dx'])
  test_dxdt = torch.Tensor(data['test_dx'])
#  ham = torch.squeeze(torch.tensor(hamiltonian_fn(np.transpose(data['x'])), requires_grad=True, dtype=torch.float32))
#  ham_test = #torch.squeeze(torch.tensor(hamiltonian_fn(np.transpose(data['test_x']))))
#  _ , hnn_ham= model.forward(x)
#  _ , hnn_ham_test=model.forward(test_x)
