...
def hamiltonian_fn(coords):
    ...
    H = 0.5*p**2 + 0.5*q**2 + 0.5*s**2 - 1 # quadratic hamiltonian (linear oscillator)
    return H
...
def get_trajectory(hamint = False,t_span=[0,15], timescale=10, radius=None, y0=None, noise_std=0.1, **kwargs):
    ...
    y0 *= radius/np.linalg.norm(y0,2) ## set the appropriate radius
    #
    if hamint:
...
def get_dataset(seed=0, samples=15, test_split=0.5, hamint=False, **kwargs):
    data = {'meta': locals()}
    ...
    for l in range(samples):
        x, y, z, dx, dy, dz, t = get_trajectory(hamint, **kwargs)
